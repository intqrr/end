...
fn cors_headers() -> Vec<tiny_http::Header> {
    vec![
        ...
        tiny_http::Header::from_bytes(
            ...
            b"Content-Range, Accept-Ranges, Content-Length",
        )
            .unwrap(),
    ]
}
...
pub fn spawn_audio_server() -> u16 {
    let server =
        tiny_http::Server::http("127.0.0.1:0")
            .expect("не удалось поднять локальный сервер");

    let port = server
        .server_addr()
        .to_ip()
        .expect("локальный адрес сервера не является IP")
        .port();
...
pub fn init_audio_server_port(port: u16) {
    AUDIO_SERVER_PORT
        .set(port)
        .expect("порт уже установлен");
}

pub fn audio_url(id: usize) -> String {
    let port = *AUDIO_SERVER_PORT
        .get()
        .expect("сервер ещё не запущен");

    format!(
        "http://127.0.0.1:{port}/track/{id}?v={}",
        get_cache_buster()
    )
}
...
pub fn video_url(id: usize) -> String {
    ...
    format!(
        "http://127.0.0.1:{port}/video/{id}?v={}",
        get_cache_buster()
    )
}
...
pub fn cover_url(id: usize, idx: usize) -> String {
    ...
    format!(
        "http://127.0.0.1:{port}/cover/{id}/{idx}?v={}",
        get_cache_buster()
    )
}
...
pub fn register_track(
    ...
    eprintln!(
        "[audio_server] register_track: id={}, audio={:?}, video={:?}, offset={}ms",
        id,
        audio_path,
        video_path,
        video_offset_ms
    ...
    track_registry().lock().unwrap().insert(
        id,
        TrackMedia {
            audio: audio_path,
            video: video_path,
...
pub fn unregister_track(id: usize) {
    eprintln!(
        "[audio_server] unregister_track: id={}",
        id
    );
